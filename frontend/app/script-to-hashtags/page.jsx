...
export default function NewProject() {
	...
	return (
		<div className={"flex flex-grow h-[90vh] flex-col justify-start  items-center"}>
			...
			{!loading ?
				img.length > 0 ?
					<div className={"grid grid-cols-4 cursor-pointer gap-5 items-center justify-center w-10/12 px-4 py-8 flex-grow"}>

						{img.map((val, ind) => {
							return (
								val.length !== 0 ?
									<div onClick={() => { navigator.clipboard.writeText(val); toast("Hashtag copied successfully") }} key={ind} className=" bg-secondary hover:bg-secondary/85 rounded-lg hover:scale-95 transition-all px-3 py-2">
